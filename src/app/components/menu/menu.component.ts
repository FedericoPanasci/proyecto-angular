...
import {
  cleanUser,
  showUser,
} from 'src/app/features/login-redux/store/login.actions';
import {
  loginSelector,
  loginStateSelector,
} from 'src/app/features/login-redux/store/login.selector';
...
export class MenuComponent implements OnInit {
  ...
  role$: string = '';
  ngOnInit(): void {
    this.state$ = this.store.pipe(select(loginSelector));
  ...
  singOut() {
    Swal.fire({
      title: 'ATENCION',
      text: 'Estas por cerrar tu sesion y perderas todas las operaciones efectuadas',
      icon: 'warning',
